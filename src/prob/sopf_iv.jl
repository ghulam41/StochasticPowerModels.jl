...
function solve_sopf_iv_n(file::String, model_constructor, optimizer; deg::Int=1, solution_processors=[sol_data_model!], kwargs...)
    data = _PM.parse_file(file)
    
    return solve_sopf_iv_n(data, model_constructor, optimizer; deg=deg, solution_processors=solution_processors, kwargs...)
end

function solve_sopf_iv_n(data::Dict, model_constructor, optimizer; deg::Int=1, solution_processors=[sol_data_model!], kwargs...)
    @assert _IM.ismultinetwork(data) == false "The data supplied is multinetwork, it should be single-network"
    @assert model_constructor <: _PM.AbstractIVRModel "This problem type only supports the IVRModel"
    
    sdata = build_stochastic_data(data, deg)
    result = _PM.solve_model(sdata, model_constructor, optimizer, build_sopf_iv_n; multinetwork=true, solution_processors=solution_processors, kwargs...)
    result["mop"] = sdata["mop"]
    
    return result
end

function build_sopf_iv_n(pm::AbstractPowerModel)
    for (n, network) in _PM.nws(pm) 
        variable_bus_voltage(pm, nw=n)
        variable_branch_current(pm, nw=n)
        variable_gen_power(pm, nw=n, bounded=false)                             # enforcing bounds alters the objective 
        ...
        variable_gen_current_magnitude_squared(pm, nw=n)
        variable_load_current_magnitude_squared(pm, nw=n)
    end
    ...
    for (n, network) in _PM.nws(pm)
        ...
        for g in _PM.ids(pm, :gen, nw=n)
            # constraint_gp_gen_power(pm, g, nw=n)
            constraint_gp_gen_power_n(pm, g, nw=n)
        end
        for l in _PM.ids(pm, :load, nw=n)
            # constraint_gp_load_power(pm, l, nw=n)
            constraint_gp_load_power_n(pm, l, nw=n)
        end
    end
